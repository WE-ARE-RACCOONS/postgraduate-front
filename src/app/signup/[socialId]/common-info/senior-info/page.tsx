...
import { photoUrlAtom, sFieldAtom, sKeywordAtom, sLabAtom, sMajorAtom, sPostGraduAtom, sProfessorAtom } from "@/stores/senior";
...
function SeniorInfoPage() {
  ...
  useEffect(() => {
    if(sPostGradu && sMajor && sLab && sProfessor && sField && sKeyword) setFlag(false);
  ...
  const handleSubmit = () => {
    ...
    if(socialId && phoneNumber && nickName && certification) {
      axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/auth/senior/signup`, {
        socialId: socialId,
  ...
  return (
    <SeniorInfoPageContainer>
      ...
      <BtnContainer>
        ...
        {flag && <SingleValidator msg={`${emptyPart}을 입력해주세요`} textColor="#FF0000" /> }
