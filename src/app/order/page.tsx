...
function Order() {
  ...
  const content = useRef<Content>({
    ...
    card_ver: '02', // DEFAULT: 01 (01: 정기결제 플렛폼, 02: 일반결제 플렛폼), 카드결제 시 필수
  ...
  const handleChange = (e: any) => {
    ...
    content.current[name as keyof Content] = value;
  };

  const handleSubmit = (e: React.FormEvent<HTMLButtonElement>) => {
    e.preventDefault();
    const queryString = `?content=${encodeURIComponent(
      JSON.stringify(content.current),
    )}`;
    const url = `/react/order_confirm${queryString}`;
    router.push(url);
  };
      ...
      <form id="orderForm" name="orderForm" onChange={handleChange}>
        ...
        <div>
          ...
          <span
            id="card_ver_view"
            style={{
              display: content.current.pay_type !== 'card' ? 'none' : '',
            }}
          >
        ...
        <div>
          ...
          <input
            type="text"
            name="payple_payer_id"
            id="payple_payer_id"
            defaultValue=""
          />
        </div>
        <div>
          <label htmlFor="payer_no">구매자 고유번호</label>{' '}
          <input
            type="text"
            name="buyer_no"
            id="buyer_no"
        ...
        <div>
          ...
          <input
            ...
            name="order_num"
            id="order_num"
            defaultValue={content.current.order_num}
          />
        </div>
...
//     $("#pay_type").on('change', function (e: any) {
//       e.preventDefault();
//       const this_val = $(e.currentTarget).val() as string;
...
//       $('#card_ver').on('change', function (this: HTMLSelectElement) {
//         if ($(this).val() === '01') {
//           $('#is_reguler_view').css('display', '');
//           $('#pay_year_view').css('display', '');
//           $('#pay_month_view').css('display', '');
...
const createOid = () => {
  ...
  now_month = now_month < 10 ? (('0' + now_month) as any) : now_month;
  let now_day = now_date.getDate();
  now_day = now_day < 10 ? (('0' + now_day) as any) : now_day;
  const datetime = now_date.getTime();
  return 'test' + now_year + now_month + now_day + datetime.toString();
};
