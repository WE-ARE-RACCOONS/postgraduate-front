...
function ProfileDonePage() {
  ...
  return(
    <ProfileDonePageContainer>
      ...
      <div id="profile-done-sub-direction">{PROFILE_SUB_DIRECTION.profileDone}</div>
      <Image id="profile-done-img" src={party_popper} alt="프로필 등록 축하 이미지" />
      <div id="profile-done-btn-container">
        <button onClick={() => {router.push('/')}}>확인했어요</button>
        <button onClick={() => {router.push('/mypage')}}>내 프로필 보러가기</button>
      </div>
    </ProfileDonePageContainer>
  )
}
