...
function useAuth() {
  ...
  function getAccessToken() {
    if(accessTkn) { // access token 있을 때
      if(isExpired(accessExp)) { // 만료됨
        if(getRefreshToken()) { // refresh token 있을 때
          reissueToken();
          return accessTkn;
        }
        if(!getRefreshToken()) { // refresh token 없을 때
          return 'required';
        }
      }
    }
  ...
  function reissueToken() {
    axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/user/refresh`, null, {
      headers: {
