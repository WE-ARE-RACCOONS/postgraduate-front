...
import { IntroCardContainer, IntroCardOneLiner, IntroCardTextBox, IntroCardTextDesc, IntroCardTimeBox } from "./IntroCard.styled";
...
function IntroCard(props: IntroCardProps) {
  ...
  const formatTime = (startTime: string, endTime: string) => {
    ...
    return `${setAmOrPm(startTimeNum)} ${startTimeNum}시 ~ ${setAmOrPm(endTimeNum)} ${endTimeNum}시`;
  }

  return(
    <IntroCardContainer>
      <IntroCardOneLiner>{props.oneLiner ? `\"${props.oneLiner}\"` : '\"잘 부탁드립니다!\"'}</IntroCardOneLiner>
      ...
      {props.times && (props.times.map((el, idx) => (
        <IntroCardTimeBox key={idx}>{el.day}요일 {formatTime(el.startTime, el.endTime)}</IntroCardTimeBox>
      )))}
    </IntroCardContainer>
  )
}
