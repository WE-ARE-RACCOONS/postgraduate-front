...
import { SMCancelTop, SMCancelMid,SMCBtn,SModalMentoringBackground,SMCBgContainer
,SMCBtnEtc,
SMCbtnCancelT,
SMCbtnCancelF
  ...
  return (
    <SMCBgContainer>
      ...
      <Image
        ...
        priority
        onClick={xClick}
        style={{ width: '2rem', height: '2rem' , marginLeft:'17.7rem',marginTop:'0.5rem'}}
      />
      <SMCancelTop>
      ...
      <SMCancelMid>
        ...
        <CheckBox checked={time} onChange={setTime} />
          {SENIOR_MENTOR_CANCEL.haveSchedul}
        </SMCBtn>
        <SMCBtn onClick={(e) => setReason(e.currentTarget.textContent ?? '')}>
        <CheckBox checked={know} onChange={setKnow} />
          {SENIOR_MENTOR_CANCEL.dontKnow}
        </SMCBtn>
        <SMCBtnEtc>
          <div style={{display:'flex'}}>
        <CheckBox checked={etc} onChange={setEtc} />
        기타
        </div>
        <div style={{display:'flex'}}>
        <input
        style={{border:'none',fontWeight: '400',height:'2rem',width:'20rem',
      ...
      {flag ? <SMCbtnCancelT onClick={cancelMentoring}>거절하기</SMCbtnCancelT > : <SMCbtnCancelF >거절하기</SMCbtnCancelF>}
      </div>
    </SModalMentoringBackground>
    </SMCBgContainer>
  );
